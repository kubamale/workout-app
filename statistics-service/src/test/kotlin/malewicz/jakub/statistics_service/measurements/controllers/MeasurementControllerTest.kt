...
class MeasurementControllerTest(
  ...
  @MockitoBean private lateinit var measurementService: MeasurementService

  private val measurementDetails =
      MeasurementDetails(
          weight = 100.0,
          bodyFat = 20.0,
          leftArm = 50.0,
  ...
  fun `get measurements should return 404 when no saved measurement for user`() {
    val userId = UUID.randomUUID()
    `when`(measurementService.getLatestMeasurement(userId))
        .thenThrow(ResourceNotFoundException::class.java)
    mockMvc
        .perform(get("/api/v1/measurements/latest").header("X-User-Id", userId))
  ...
  fun `get measurements since should return 200 when passed correct data`() {
    ...
    `when`(measurementService.getMeasurementsSince(userId, fromDate))
        .thenReturn(mutableListOf(measurementDetails))
